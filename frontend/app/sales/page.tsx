...
function SalesContent() {
  ...
  const [recordUdhaar, setRecordUdhaar] = useState(true);

  function load() {
    api.get<Order[]>("/sale-orders").then(setOrders).catch(() => {});
  }
  ...
  function addItemRow() {
    ...
    setItems([...items, { product_id: products[0].id, qty: 1, unit_price: products[0].sale_price }]);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const order = await api.post<Order>("/sale-orders", { party_id: partyId, date: today, items });

    if (recordUdhaar) {
      await api.post("/ledger/entries", {
        party_id: partyId, date: today, type: "debit", amount: order.total, method: "udhaar", ref_order_id: order.id,
      });
    }
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} className="mt-4 rounded-lg border border-black/10 p-4 dark:border-white/10">
        ...
        <label className="mt-3 flex items-center gap-2 text-sm">
          <input type="checkbox" checked={recordUdhaar} onChange={(e) => setRecordUdhaar(e.target.checked)} />
          Record as udhaar (credit) in the khata
        </label>
